...
: (∃ L : ℝ, Tendsto (fun N ↦ ∑ j in Finset.Icc 2 N, f j) ⊤ (𝓝 L)) ↔ putnam_1982_a2_solution := by
sorry

noncomputable abbrev putnam_1982_a3_solution : ℝ := sorry
-- (Real.pi / 2) * log Real.pi
theorem putnam_1982_a3
: (Tendsto (fun t ↦ ∫ x in (0)..t, (arctan (Real.pi * x) - arctan x) / x) ⊤ (𝓝 putnam_1982_a3_solution)) := by
sorry
